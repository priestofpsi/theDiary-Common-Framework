        ...
        public virtual string ToHtmlString()
        {
            ...
            else
            {
                ...
                foreach (var paginationLink in model.PaginationLinks)
                {
                    ...
                    {
                        if (!paginationLink.PageIndex.HasValue)
                        {
                            displayText = paginationLink.DisplayText;
                        }
                        else
                        {
                            ...
                            linkBuilder.AppendFormat(" href=\"{0}\" class=\"{2}\">{1}</a>", paginationLink.Url, paginationLink.DisplayText, this.pagerOptions.ItemElementAdditionalClass);
                            this.AddClass(ref @class, paginationLink.IsCurrent, "current");
                            displayText = linkBuilder.ToString();
                        }
                    }
                    else
                    {
                        if (paginationLink.IsArrow)
                        {
                            displayText = string.Format("<a href=\"#\">{0}</a>", paginationLink.DisplayText);
                        }
                        else
                        {
                            displayText = paginationLink.DisplayText;
                        }
                        this.AddClass(ref @class, paginationLink.IsArrow, this.pagerOptions.PagingClass);

                        this.AddClass(ref @class, paginationLink.IsSpacer, "spacer", this.pagerOptions.DisabledClass);
                        
                    }
                    sb.AppendFormat("<{2} class=\"{1}\">{0}</{2}>", displayText, @class, this.pagerOptions.PageItemElement);
                }
                return sb.ToString();
            }
        }

        private void AddClass(ref string @class, bool condition, string trueCondition)
        {
            this.AddClass(ref @class, condition, trueCondition, string.Empty);
        }

        private void AddClass(ref string @class, bool condition, string trueCondition, string falseCondition)
        {
            this.AddClass(ref @class, condition ? trueCondition : falseCondition);
        }
        private void AddClass(ref string @class, string classToAdd)
        {
            if (classToAdd.IsNullEmptyOrWhiteSpace())
                return;

            if (@class == null)
                @class = string.Empty;

            @class += string.Format(" {0}", classToAdd);
        }
